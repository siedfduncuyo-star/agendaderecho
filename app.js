...
function bindEvents() {
  ...
  [importDialog, detailDialog, calendarDialog, reportDialog, bulkDatesDialog, hibridacionesDialog, gradeScheduleDialog, legalClinicsDialog, mediationCenterDialog, programsDialog].forEach((dialog) => dialog.addEventListener("click", (event) => { if (event.target === dialog) dialog.close(); }));
...
async function loadPeriod() {
  ...
  updatePeriodTitle(); updateNavigationState(); updateBulkAcademicDatesVisibility(); updateCleanupAgendaVisibility(); updateBulkHibridacionesVisibility(); updateBulkGradeScheduleVisibility(); updateBulkLegalClinicsVisibility(); updateBulkMediationCenterVisibility(); updateBulkProgramsVisibility(); render();
}
