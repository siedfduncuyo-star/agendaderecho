...
function bindEvents() {
  ...
  [importDialog, detailDialog, calendarDialog, reportDialog, bulkDatesDialog, hibridacionesDialog, gradeScheduleDialog, legalClinicsDialog, mediationCenterDialog, programsDialog, ingreso2027Dialog].forEach((dialog) => dialog.addEventListener("click", (event) => { if (event.target === dialog) dialog.close(); }));
...
async function loadPeriod() {
  ...
  try {
    if (configured) {
      ...
      state.programsBulkLoaded = Boolean(configRecord?.[programsBulkMarker]);
      state.ingreso2027UpdateLoaded = Boolean(configRecord?.[ingreso2027UpdateMarker]);
...
function groupChildTitle(group, item) {
  if (group.group_kind === "grade") return subjectBaseName(item.subject) || item.name;
...
function createActivityGroupContent(group) {
  ...
  summary.textContent = `${group.items.length} ${group.group_kind === "grade" ? "clases" : "actividades"} · ${displayGroupTime(group.items)}${rooms ? ` · ${rooms}` : ""}`;
